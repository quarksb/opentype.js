...
function arraysEqual(ar1: any[], ar2: any[]): boolean {
    const n = ar1.length;
...
function binarySearch(array: any[], key: string, value: any): any | null {
...
function binarySearchIndex(array: any[], key: string, value: any): number {
    let low = 0, high = array.length - 1;
    while (low <= high) {
        const mid = Math.floor((low + high) / 2);
...
function binarySearchInsert(array: any[], key: string, value: any): number {
    let low = 0, high = array.length;
    const compare = (a: any, b: any) => a[key] - b[key];
    while (low < high) {
 ...
 * [GZIP File Format Specification](https://datatracker.ietf.org/doc/html/rfc1952#section-2.3)
 */
function unGzip(gzip: Uint8Array): Uint8Array {
    ...
    inflate(gzip.subarray(start, end), dest);
    return dest;
}

function copyPoint(p: any): any {
    return {
        x: p.x,
        y: p.y,
        onCurve: p.onCurve,
...
function copyComponent(c: any): any {
    return {
